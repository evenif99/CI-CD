...
from ch15_ml.imageDataGeneratorTest import IMAGE_WIDTH, IMAGE_HEIGHT, x_train
...
IMG_WIDTH, IMG_HEIGHT = 224, 224 # VGG16 모델의 이미지 정보
...
# include_top=False : 1000개의 클래스를 무시하고, 내가 10개로 하겠습니다.
# input_shape : 입력 이미지 크기를 지정하는 부분입니다.
...
# 다만 우리가 다시 추가하는 Dense 층들만 학습시키겠습니다.

from keras import Model
from keras.src.layers import Dense, MaxPooling2D, Dropout

# Custom Classified 생성
x = model.output
x = MaxPooling2D()(x)

# activation : 활성화 함수
# 마지막 레이어가 아닌 레이어에서는 주로 'relu' 활성화 함수를 많이 사용합니다.
x = Dense(units = 256, activation='relu')(x)

x = Dropout(0.5)(x) # 노드의 일부를 off
x = Dense(units = 128, activation='relu')(x)

# 마지막 레이어의 노드 개수(units)는 클래스 개수와 동일해야 합니다.
...
# 출력 : (None, 10) 개발자가 정의한 클래스 개수가 보입니다.
...
for filename in file_list:
    ...
    current_image = load_img(myfile, target_size=(IMG_WIDTH, IMG_HEIGHT))
    plt.imshow(current_image)
    plt.axis('off')

    save_file = image_destination + 'pretrained_model_' + filename
...
# image_size = prediction[0].shape
# class_size = prediction.shape[1]
#
# print(f'\n입력 이미지의 개수는{image_size}이고, ', end='')
# print(f'\n클래스의 개수는{class_size}입니다.')
# print('prediction에는 확률 값이 들어있습니다.')
...
#     print(f'이미지 `{file_list[idx]}`의 확률값 정보를 수집합니다.')
