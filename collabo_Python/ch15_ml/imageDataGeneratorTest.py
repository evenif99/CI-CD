...
print(f'한번에 실행할 사이즈 : {iters.batch_size}') # batch_size 속성과 연관있음
print(f'이미지 형상 정보 : {iters.image_shape}') # target_size와 color_mode와 연관있음
print(f'클래스 레이블과 색인 정보 : {iters.class_indices}')

print('next() 함수는 반복자(iterator) 객체인 iters에서 다음 항목을 읽어주는 파이썬 내장 함수입니다.')
