...
print(f'\n이 모델은 클래스의 개수가 {len(survived_counts)}개입니다.')
...
print(f'\n승선 위치 컬럼에서 가장 빈도가 큰 항목 : {most_frequency}')
print(f'컬럼 embarked의 결측치를 {most_frequency} 값으로 채워줍니다.')
...
print('# 종속 변수 : 독립 변수에 의하여 영향을 받는는 변수(output 데이터 : 답지)')
...
model.fit(x_train, y_train) # 시험지와 답지

print('\n# predict() 함수는 가장 높은 확률의 클래스 값(0 또는 1)을 반환해 줍니다.')
prediction = model.predict(x_test) # x_test는 답지 없이 시험 본 나의 문제지

print('\n# 예측 값(모의고사에서 내가 작성한 답지)')
print(prediction[0:10])

print('\n# 실제 정답 데이터(모의고사의 진짜 답지_label)')
...
print('\n# 분류 모델 성능 평가')
print('# confusion_matrix(실제 정답 데이터, 예측값')
